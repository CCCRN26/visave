...
export const digitalAccessSchema=z.discriminatedUnion('mode',[
  z.object({mode:z.literal('CREATE_USER'),firstName:z.string().trim().min(1).max(100),lastName:z.string().trim().min(1).max(100),email:z.string().trim().toLowerCase().email(),phone:z.string().trim().max(30).optional(),password:z.string().min(12).max(128)}),
